...
image.addEventListener("click", () => {
    ...
    async function imageMovie() {
        ...
        const image = await fetch(`https://api.themoviedb.org/3/movie/${videoId}/images`, detail);
...
video.addEventListener("click", () => {
    ...
    async function videoMovie() {
        ...
        const image = await fetch(`https://api.themoviedb.org/3/movie/${videoId}/videos?language=en-US`, detail);
        const data = await image.json();
        return data
    ...
    async function displayVideoMovie() {
        ...
        console.log(result);
        video.innerText = `Video |  ${result.length}`
...
// let videoId = sessionStorage.getItem("video-id")
...
async function detailMovie() {
    ...
    const details = await fetch(`https://api.themoviedb.org/3/movie/${videoId}?language=en-US`, detail);
...
async function displayDetail() {
    const videoDetail = await detailMovie()

    const watch = localStorage.getItem("list")
    // cards.style.backgroundImage = `url(${imgUrl}${videoDetail.poster_path})`
    if (watch != null) {
        if (watch.includes(videoDetail.id) == false) {

            cards.innerHTML += `
                            <div  id="cards-detail">
                                    <div class="img">
                                    <img src="${imgUrl}${videoDetail.poster_path}" alt="">
                                    <button class="w-25"   onclick="setWatchList(${videoDetail.id})"><i class="fa-solid fa-bookmark border-0" style="color :#ffd43b4d"></i></button>
                                </div>
                                <div class="info">
                                <h2>${videoDetail.title}</h2>
                                <h5>Release Date : ${videoDetail.release_date}</h5>
            ...
            cards.innerHTML += `
                            <div  id="cards-detail">
                                    <div class="img">
                                    <img src="${imgUrl}${videoDetail.poster_path}" alt="">
                                    <button class="w-25"   onclick="setWatchList(${videoDetail.id})"><i class="fa-solid fa-bookmark border-0" style="color : #FFD43B" ></i></button>
                                </div>
                                <div class="info">
                                <h2>${videoDetail.title}</h2>
                                <h5>Release Date : ${videoDetail.release_date}</h5>
...
async function creditsMovie() {
    ...
    const cast = await fetch(`https://api.themoviedb.org/3/movie/${videoId}/credits?language=en-US`, detail);
...
const avatar = "https://image.tmdb.org/t/p/w45_and_h45_face"

async function reviews() {

    const rev = await fetch(`https://api.themoviedb.org/3/movie/${videoId}/reviews?language=en-US`, detail);
    ...
    if (result.length == 0) {
        ...
        for (let review of result.slice(0, 1)) {
            carousel.innerHTML += ` <div class="carousel-item review w-50 active " >
                                    <div class="user-info">
                                        <img class="user-avatar" src="${avatar}${review.author_details.avatar_path}" alt="User Avatar">
...
async function displayImageMovie() {
    ...
    image.innerText = `Images |  ${imagesMovie.backdrops.length}`
    for (let image of imagesMovie.backdrops) {
...
async function recommendations() {
    ...
    const image = await fetch(`https://api.themoviedb.org/3/movie/${videoId}/recommendations?language=en-US`, detail);
    const data = await image.json();
    return data
...
async function displayRecommendations() {
    ...
    for (let recommend of result) {
        cardsRec.innerHTML += ` <div class="card" id="${recommend.id}">
                   
                    <img src="${imgUrl}${recommend.poster_path}" alt="">
                    <div class = "movie-title">
                    <a href="../details.html" onclick="getId(${recommend.id})">${recommend.title}</a>
