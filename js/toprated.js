...
async function topRated() {
    ...
    const rated = await fetch(`https://api.themoviedb.org/3/movie/top_rated?language=en-US&page=${currentPage}region=arabic`, options);
...
async function displayTopRated() {
            ...
            if (watch.includes(movie.id) == false) {
                cards.innerHTML += ` <div class="card-content" >
                                        <div class="card">
                                            ...
                                            <a href="../details.html" onclick = "getId(${movie.id})">${movie.title.split(" ").slice(0, 2).join(" ")}</a>
                   ...
                   <div class="card">
                       <div class="overlay"><i class="fa-regular fa-circle-play svg" style="color: #e8ebef;"></i></div>
                           ...
                           <div class = "movie-title" >
                            <button class="w-25 button"  onclick="setWatchList(${movie.id})"><i class="fa-solid fa-bookmark border-0" style="color :#ffd43b4d"></i></button>
                            <a href="../details.html" onclick="getId(${movie.id})">${movie.title.split(" ").slice(0, 2).join(" ")}....</a>
                            <p> <i class="fa-solid fa-star" style="color: #ffbb00;"></i>  ${movie.vote_average}</p>
                       </div>
                   </div>
                </div>`
        }
    ...
    //     cards.innerHTML += `
    //                      <div class="card-content" id="${rate.id}">
    //                             <div class="card">
    //                                 <div class="overlay"><i class="fa-regular fa-circle-play svg" style="color: #e8ebef;"></i></div>
    //                                     <img src="${imgPath}${rate.poster_path}" alt="">
    //                                     <div class = "movie-title" >
    //                                     <a href="../details.html" onclick="getId(${rate.id})">${rate.title.split(" ").slice(0, 2).join(" ")}....</a>
    //                                     <p> <i class="fa-solid fa-star" style="color: #ffbb00;"></i>  ${rate.vote_average}</p>
